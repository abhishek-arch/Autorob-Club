...
const upload = require("../config/multer.config")
 router.post("/register",async(req,res)=>{
  ...
  const user = await createUser({fullname:{firstname,lastname},
        email,
...
router.get('/profile',authMiddleware.authenticateUser, async(req,res)=>{
 ...
 router.post('/dashboard',authMiddleware.authenticateUser,async(req,res)=>{
    ...
    const existingUser = await userdashboardModel.findOne({email});
    if(existingUser) {
      return res.status(400).json({message: "User with this email already exists"});
    
    }
  
  const userdashboard = await createuserdasboard({fullname:{firstname,lastname},
        email,
        profilephoto,
        expertise,
        phone,
        ...
        res.status(201).json({message: "Userdashboard created successfully", userdashboard});
 ...
 router.post('/upload',authMiddleware.authenticateUser,upload.single("image"),async(req,res)=>{
